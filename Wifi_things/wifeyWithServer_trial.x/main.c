...
#endif
{
    ...
    while (1) {
        ...
        if (g_WpsPassphrase.valid) {
            WF_DeriveKey(g_WpsPassphrase.passphrase.keyLen, g_WpsPassphrase.passphrase.key,
        ...
        if (AppConfig.hibernateFlag == 1) 
            AppConfig.hibernateFlag = 0;
    }
}
...
static void ProcessIO(void)
{
    ...
    tcpip_helper_uitoa((uint16_t) ADC1BUF0, AN0String);
#else
    ...
    tcpip_helper_uitoa(*((uint16_t *)(&ADRESL)), AN0String);
#endif
}
